...
```swift
var humanClassObject = HumanClass(name: "NiKS")
humanClassObject.name // "NiKS"
```
...
Change the `name` property of `newHumanClassObject` to "Nikhil".

```swift
humanClassObject.name = "Nikhil"
newHumanClassObject.name // "Nikhil"
```

The `name` property of `newHumanClassObject` has been changed to "Nikhil" as well.
...
```swift
var structObject = HumanStruct(name: "Niks")
let newStructObject = structObject
```

Change the `name` property of `structObject` to "Nikhil".

```swift
structObject.name = "Nikhil"
newStructObject.name // "Niks"
```
...
niksClass.name = "Nikhil"

let newNiKSClass = NiKSClass

newNiKSClass.name = "NiKS the Developer"
NiKSClass.name // "NiKS the Developer"
```

Although the instance, `NiKSClass` has been created with `let`, the property of it has been mutated. It is possible since the instance does not own the object. Instead, it has a reference to the`NiKSClass` object in the cloud/RAM.
...
[Intro to Protocol Oriented Programming]:  https://blog.NiKSthedeveloper.io/introduction-to-protocol-oriented-programming-in-swift-b358fe4974f
